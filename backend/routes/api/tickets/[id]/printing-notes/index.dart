...
/// PUT /api/tickets/{id}/printing-notes - Update ticket printing notes
Future<Response> _handlePut(RequestContext context, String id) async {
  try {
    ...
    final validationError = _validateUpdatePrintingNotesRequest(body);
    ...
    // Update ticket printing notes
    await DatabaseService.query(
      '''
      UPDATE tickets 
      SET printing_notes = ?, updated_at = NOW()
      WHERE id = ?
      ''',
      parameters: [
        body['printingNotes'],
    ...
    // Log activity for ticket printing notes update (Activity ID: 503)
    try {
      await ActivityLogService.log(
        entityId: 3, // tickets entity
        recordId: ticketId,
        activityId: 503, // Update ticket printing notes
        userId: userId,
      );
    } catch (e) {
      print('Failed to log ticket printing notes update activity: $e');
    }
    ...
    return Response.json(
      body: {
        'success': true,
        'data': ticketData,
        'message': 'Ticket printing notes updated successfully'
      },
    );
    
  } catch (e) {
    print('Update ticket printing notes error: $e');
    return Response.json(
      statusCode: 500,
      body: {
        'success': false,
        'message': 'Internal server error occurred while updating ticket printing notes',
        'error': e.toString()
      },
    );
  }
...
String? _validateUpdatePrintingNotesRequest(Map<String, dynamic> body) {
  // printing_notes can be null or string
  if (body['printingNotes'] != null && body['printingNotes'] is! String) {
    return 'printingNotes must be a string if provided';
  }
  ...
  try {
    final ticketResult = await DatabaseService.queryOne(
      ...
      SELECT 
        ...
        t.closed_at, t.closing_notes, t.printing_notes, t.created_at, t.updated_at,
    ...
    final printingNotes = ticketResult['printing_notes'];
    ...
    return {
      ...
      'printing_notes': printingNotes is Blob ? utf8.decode(printingNotes.toBytes()) : printingNotes,
