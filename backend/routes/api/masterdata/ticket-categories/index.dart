...
Future<Response> _handleGet(RequestContext context) async {
  try {
    final results = await DatabaseService.query(
        'SELECT id, name, created_by, created_at, updated_at, company_id FROM ticket_categories',
    ...
    print('Get ticket categories error: $e');
    return _addCorsHeaders(Response.json(
      statusCode: 500,
      body: {
        'success': false,
        'message': 'Internal server error occurred while fetching ticket categories',
        'error': e.toString(),
      },
    ));
  }
...
Future<Response> _handlePost(RequestContext context) async { 
  try {
    ...
    await DatabaseService.query(
      'INSERT INTO ticket_categories (name, created_by, created_at, updated_at, company_id) VALUES (?, ?, ?, ?, ?)',
      parameters: [name, createdBy, now, now, companyId],
      userId: userId,
    );

    return _addCorsHeaders(Response.json(
      body: {'success': true, 'message': 'Ticket category created successfully'},
    ));
  } catch (e) {
    print('Post ticket category error: $e');
    return _addCorsHeaders(Response.json(
      statusCode: 500,
      body: {
        'success': false,
        'message': 'Internal server error occurred while creating ticket category',
        'error': e.toString(),
      },
    ));
  }
