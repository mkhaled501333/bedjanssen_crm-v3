...
Future<Response> _handleGet(RequestContext context, String id) async {
  try {
    final result = await DatabaseService.query(
      'SELECT id, name, created_by, created_at, updated_at, company_id FROM ticket_categories WHERE id = ?',
      parameters: [id],
    ...
    if (result.isEmpty) {
      return Response.json(
        statusCode: 404,
        body: {'success': false, 'message': 'Ticket category not found'},
      );
    }
    ...
    print('Get ticket category by id error: $e');
    return Response.json(
      statusCode: 500,
      body: {
        'success': false,
        'message': 'Internal server error occurred while fetching ticket category',
        'error': e.toString(),
      },
    );
  }
...
Future<Response> _handlePut(RequestContext context, String id) async {
  try {
    ...
    await DatabaseService.query(
      'UPDATE ticket_categories SET name = ?, updated_at = ? WHERE id = ?',
      parameters: [name, now, id],
      userId: userId,
    );

    return Response.json(
      body: {'success': true, 'message': 'Ticket category updated successfully'},
    );
  } catch (e) {
    print('Put ticket category error: $e');
    return Response.json(
      statusCode: 500,
      body: {
        'success': false,
...
Future<Response> _handleDelete(RequestContext context, String id) async {
  try {
    ...
    return Response.json(
      statusCode: 200,
      body: {'success': true, 'message': 'Ticket category deleted successfully'},
    );
  } catch (e) {
    print('Delete ticket category error: $e');
    return Response.json(
      statusCode: 500,
      body: {
        'success': false,
        'message': 'Internal server error occurred while deleting ticket category',
        'error': e.toString(),
      },
    );
  }
