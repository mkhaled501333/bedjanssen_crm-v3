...
Future<Response> _handlePut(RequestContext context, String itemId) async {
  try {
    ...
    if (body.containsKey('productSize')) {
      queryParts.add('product_size = ?');
      params.add(body['productSize']);
    }
    if (body.containsKey('cost')) {
      queryParts.add('cost = ?');
      params.add(body['cost']);
    }
    ...
    if (body.containsKey('pulled')) {
      ...
      specialActivities.add(pulledValue == 1 ? 606 : 638); // 606: pulled, 638: unpulled
    }
    ...
    if (body.containsKey('delivered')) {
      ...
      specialActivities.add(deliveredValue == 1 ? 608 : 639); // 608: delivered, 639: undelivered
    }
    ...
    final result = await DatabaseService.query(
      'UPDATE ticket_item_change_another SET ${queryParts.join(', ')} WHERE ticket_item_id = ?',
      parameters: params,
      userId: userId,
    );
    ...
    if (result.affectedRows == 0) {
      return Response.json(
        statusCode: 404,
        body: {'success': false, 'message': 'Record not found or no changes made'},
      );
    }
    ...
    return Response.json(
      body: {'success': true, 'message': 'Details updated successfully'},
    );
...
Future<Response> _handlePost(RequestContext context, String itemId) async {
  try {
    ...
    await DatabaseService.query(
      '''
      INSERT INTO ticket_item_change_another (ticket_item_id, company_id, product_id, product_size, cost, client_approval, refusal_reason, pulled, pull_date, delivered, delivery_date, created_by, created_at, updated_at) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
      ''',
      parameters: [
        itId,
    ...
    await ActivityLogService.log(
      entityId: 4, // ticket_items
      recordId: itId,
      activityId: 600, // Create different product replacement
      userId: userId,
    );

    return Response.json(
      statusCode: 201,
      body: {'success': true, 'message': 'Different product replacement selected successfully'},
    );
...
Future<Response> _handleDelete(RequestContext context, String itemId) async {
  try {
    ...
    await DatabaseService.query(
      'DELETE FROM ticket_item_change_another WHERE ticket_item_id = ?',
      parameters: [itId],
      userId: userId,
    );
    ...
    await ActivityLogService.log(
      entityId: 4, // ticket_items
      recordId: itId,
      activityId: 610, // Delete different product replacement
      userId: userId,
    );

    return Response.json(
      body: {'success': true, 'message': 'Different product replacement deleted successfully'},
    );
