...
Future<Response> _handlePut(RequestContext context, String itemId) async {
  try {
    ...
    if (body.containsKey('pulled')) {
      ...
      specialActivities.add(pulledValue == 1 ? 617 : 640); // 617: pulled, 640: unpulled
    }
    ...
    if (body.containsKey('delivered')) {
      ...
      specialActivities.add(deliveredValue == 1 ? 619 : 641); // 619: delivered, 641: undelivered
    }
...
Future<Response> _handlePost(RequestContext context, String itemId) async {
  try {
    ...
    await DatabaseService.query(
      '''
      INSERT INTO ticket_item_change_same (ticket_item_id, company_id, product_id, product_size, cost, client_approval, refusal_reason, pulled, pull_date, delivered, delivery_date, created_by, created_at, updated_at) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
      ''',
      parameters: [
        itId,
    ...
    await ActivityLogService.log(
      entityId: 4, // ticket_items
      recordId: itId,
      activityId: 611, // Create same product replacement
      userId: userId,
    );

    return Response.json(
      statusCode: 201,
      body: {'success': true, 'message': 'Same product replacement selected successfully'},
    );
...
Future<Response> _handleDelete(RequestContext context, String itemId) async {
  try {
    ...
    await ActivityLogService.log(
      entityId: 4, // ticket_items
      recordId: itId,
      activityId: 621, // Delete same product replacement
      userId: userId,
    );

    return Response.json(
      body: {'success': true, 'message': 'Same product replacement deleted successfully'},
    );
